...
import { BattleLogFilterSchema, BattleLogIdParamSchema } from "@/modules/battle-log/validators/battle-log.validator.js";
...
export const battleLogRoutes = {
    battleLogIndex: authEndpoint(`${prefix}`, { method: "GET", query: BattleLogFilterSchema }, (ctx) =>
        RouterContainer(ctx, async () => {
            const records = await battleLogService.getList(ctx.query);
            return Response.ok({ data: records });
        })
    ),
    battleLogShow: authEndpoint(`${prefix}/:id`, { method: "GET", params: BattleLogIdParamSchema }, (ctx) =>
        RouterContainer(ctx, async () => {
