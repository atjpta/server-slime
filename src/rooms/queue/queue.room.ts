...
import { Client, IRoomCache, ServerError, debugMatchMaking, matchMaker } from "colyseus";
...
export class QueueRoom extends BaseRoomPlayer {
    maxPlayers = 4;
    maxTeamSize: number;
    ...
    protected onGroupReady = (group: QueueMatchGroup) =>
        matchMaker.createRoom(this.matchRoomName, {});
    ...
    onCreate(options: QueueOptions) {
        ...
        if (options.matchRoomName) {
            ...
            throw new ServerError(
                ...
                "QueueRoom: 'matchRoomName' option is required."
            );
        }

        debugMatchMaking(
            "QueueRoom#onCreate() maxPlayers: %d, maxWaitingCycles: %d, maxTeamSize: %d, allowIncompleteGroups: %d, roomNameToCreate: %s",
            this.maxPlayers,
            this.maxWaitingCycles,
            this.maxTeamSize,
            this.allowIncompleteGroups,
            this.matchRoomName
        );
    ...
    redistributeTeams(sortedClients: Client<{ userData: QueueClientData }>[]) {
        ...
        do {
            ...
            teams = teams.filter((team) => {
                ...
                currentGroup = this.redistributeClients(
                    team.clients.splice(0, this.maxTeamSize),
                    currentGroup,
                    totalRank
                );
